      ...
      <a-table ref="TableInfo"
        ...
        <template slot="email" slot-scope="text, record">
          <a-popover placement="topLeft">
            ...
            <p style="width: 40px;margin-bottom: 0">{{text}}&ensp;&ensp;&ensp;</p>
          </a-popover>
        </template>
        <template slot="operation" slot-scope="text, record">
          <a v-hasPermission="['user:update']" @click="edit(record)">修改用户</a>
          &nbsp;
          <a v-hasPermission="['user:view']" @click="view(record)">查看</a>
          <a v-hasNoPermission="['user:update','user:view']">无权限</a>
        </template>
      </a-table>
    </div>
    <!-- 用户信息查看 -->
